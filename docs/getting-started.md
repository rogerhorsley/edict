...
> 从零开始，5 分钟搭建你的三省七部 AI 协同系统

---

## 第一步：安装 OpenClaw

三省七部基于 [OpenClaw](https://openclaw.ai) 运行，请先安装：
...
这是最常见的问题。三省七部有 11 个 Agent，每个都需要 API Key。

```bash
# 方法一：为任意 Agent 配置后重新运行 install.sh（推荐）
openclaw agents add taizi          # 按提示输入 Anthropic API Key
cd agents_company_for_happycapy && ./install.sh            # 自动同步到所有 Agent
...
- [🏠 项目首页](https://github.com/rogerhorsley/agents_company_for_happycapy)
- [📖 README](../README.md)
