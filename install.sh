#!/bin/bash
# ══════════════════════════════════════════════════════════════
# 三省七部 · OpenClaw Multi-Agent System 一键安装脚本
# ══════════════════════════════════════════════════════════════
set -e
...
banner() {
  echo ""
  echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║  🏛️  三省七部 · OpenClaw Multi-Agent    ║${NC}"
  echo -e "${BLUE}║       安装向导                            ║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
  echo ""
}
...
create_workspaces() {
  ...
  AGENTS=(strategy review execution research hr finance brand security compliance tech)
...
AGENTS = [
  {"id": "strategy",   "subagents": {"allowAgents": ["review", "execution"]}},
  {"id": "review",     "subagents": {"allowAgents": ["strategy", "execution"]}},
  {"id": "execution",  "subagents": {"allowAgents": ["strategy", "review", "research", "hr", "finance", "brand", "security", "compliance", "tech"]}},
  {"id": "research",   "subagents": {"allowAgents": ["strategy"]}},
  {"id": "hr",         "subagents": {"allowAgents": ["execution"]}},
  {"id": "finance",    "subagents": {"allowAgents": ["execution"]}},
  {"id": "brand",      "subagents": {"allowAgents": ["execution"]}},
  {"id": "security",   "subagents": {"allowAgents": ["execution"]}},
  {"id": "compliance", "subagents": {"allowAgents": ["execution"]}},
  {"id": "tech",       "subagents": {"allowAgents": ["execution"]}},
]
...
tasks = [
    {
        ...
        "ac": "系统正常运行",
        "flow_log": [
            {"at": "2024-01-01T00:00:00Z", "from": "用户", "to": "谋部", "remark": "下达指令：初始化三省七部系统"},
            {"at": "2024-01-01T00:01:00Z", "from": "谋部", "to": "策枢", "remark": "研究完成，提交策枢"},
            {"at": "2024-01-01T00:02:00Z", "from": "策枢", "to": "衡枢", "remark": "方案提交审核"},
            {"at": "2024-01-01T00:03:00Z", "from": "衡枢", "to": "行枢", "remark": "✅ 审核通过"},
            {"at": "2024-01-01T00:04:00Z", "from": "行枢", "to": "技部", "remark": "派发：系统初始化"},
            {"at": "2024-01-01T00:05:00Z", "from": "技部", "to": "行枢", "remark": "✅ 完成"},
        ]
    }
]
import os
...
sync_auth() {
  ...
  if [ -z "$MAIN_AUTH" ] || [ ! -f "$MAIN_AUTH" ]; then
    ...
    echo "    openclaw agents add research"
...
build_frontend() {
  ...
  if ! command -v node &>/dev/null; then
    ...
    warn "请安装 Node.js 18+ 后运行: cd app/frontend && npm install && npm run build"
    return
  fi

  if [ -f "$REPO_DIR/app/frontend/package.json" ]; then
    ...
    warn "未找到 app/frontend/package.json，跳过前端构建"
  fi
}
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  🎉  三省七部安装完成！                          ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════╝${NC}"
echo ""
echo "下一步："
echo "  1. 配置 API Key（如尚未配置）:"
echo "     openclaw agents add research  # 按提示输入 Anthropic API Key"
