...
OFFICIALS = [
    {'id':'research',  'label':'谋部','role':'战略研究总监','emoji':'🔭','rank':'VP'},
    {'id':'strategy',  'label':'策枢','role':'首席战略官',  'emoji':'🧠','rank':'C-Suite'},
    {'id':'review',    'label':'衡枢','role':'首席审核官',  'emoji':'⚖️','rank':'C-Suite'},
    {'id':'execution', 'label':'行枢','role':'首席运营官',  'emoji':'🎯','rank':'C-Suite'},
    {'id':'hr',        'label':'人部','role':'人才总监',    'emoji':'👥','rank':'Director'},
    {'id':'finance',   'label':'财部','role':'财务总监',    'emoji':'💰','rank':'Director'},
    {'id':'brand',     'label':'品部','role':'品牌总监',    'emoji':'📣','rank':'Director'},
    {'id':'security',  'label':'安部','role':'安全总监',    'emoji':'🛡️','rank':'Director'},
    {'id':'compliance','label':'规部','role':'合规总监',    'emoji':'📋','rank':'Director'},
    {'id':'tech',      'label':'技部','role':'技术总监',    'emoji':'⚙️','rank':'Director'},
]
...
def get_model(agent_id):
    ...
    # 兼容历史：research 曾使用 main 作为运行时 id
...
def scan_agent(agent_id):
    ...
    if not sj.exists() and agent_id == 'research':
...
def get_task_stats(org_label, tasks):
    ...
    return {'tasks_done':len(done),'tasks_active':len(active),
            'flow_participations':fl,'participated_tasks':participated}
    ...
    for off in OFFICIALS:
        ...
        result.append({
            **off,
            ...
            'participated_tasks': ts['participated_tasks'],
