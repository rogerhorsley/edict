#!/usr/bin/env python3
"""
看板任务更新工具 - 供各部门 Agent 调用

本工具操作 data/tasks_source.json（JSON 看板模式）。
如果您已部署 app/backend（Postgres + Redis 事件总线模式），
请使用 app/backend API 端点代替本脚本，或运行迁移脚本：
  python3 app/migration/migrate_json_to_pg.py
...
用法:
  ...
  # 添加流转记录
  python3 kanban_update.py flow JJC-20260223-012 "谋部" "策枢" "研究报告提交策划"
...
STATE_ORG_MAP = {
    'Research': '谋部', 'Strategy': '策枢', 'Review': '衡枢', 'Execution': '行枢',
...
_AGENT_LABELS = {
    'main': '谋部', 'research': '谋部',
    'strategy': '策枢', 'review': '衡枢', 'execution': '行枢',
    'hr': '人部', 'finance': '财部', 'brand': '品部',
    'security': '安部', 'compliance': '规部', 'tech': '技部',
}
...
_VALID_TRANSITIONS = {
    'Pending':   {'Research', 'Cancelled'},
    'Research':  {'Strategy', 'Cancelled'},
    'Strategy':  {'Review', 'Cancelled'},
    'Review':    {'Execution', 'Strategy', 'Cancelled'},   # 驳回可回策枢
    'Execution': {'Doing', 'Blocked', 'Cancelled'},
    'Doing':     {'Done', 'Blocked', 'Cancelled'},
    'Blocked':   {'Doing', 'Execution', 'Cancelled'},      # 解除后回原位
    'Done':      set(),       # 终态
    'Cancelled': set(),       # 终态
}
