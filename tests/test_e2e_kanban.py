...
def test_dirty_title_cleaned():
    cmd_create('JJC-TEST-E2E-01',
        '全面审查/Users/bingsen/clawd/agents-company/这个项目\nConversation info (xxx)',
        'Strategy', '策枢', '首席战略官',
        '任务（自动预建）：全面审查/Users/bingsen/clawd/项目')
...
def test_pure_path_rejected():
    cmd_create('JJC-TEST-E2E-02', '/Users/bingsen/clawd/agents-company/', 'Strategy', '策枢', '首席战略官')
    assert _get_task('JJC-TEST-E2E-02') is None, "纯路径标题应被拒绝"


# ── TEST 3: 正常标题正常创建
def test_normal_title():
    cmd_create('JJC-TEST-E2E-03', '调研工业数据分析大模型应用方案', 'Strategy', '策枢', '首席战略官', '谋部整理任务')
...
def test_flow_remark_cleaned():
    cmd_create('JJC-TEST-E2E-04', '调研工业数据分析大模型应用方案', 'Strategy', '策枢', '首席战略官')
    cmd_flow('JJC-TEST-E2E-04', '谋部', '策枢', '任务传达：审查/Users/bingsen/clawd/xxx项目 Conversation blah')
...
def test_short_title_rejected():
    cmd_create('JJC-TEST-E2E-05', '好的', 'Strategy', '策枢', '首席战略官')
    assert _get_task('JJC-TEST-E2E-05') is None, "短标题应被拒绝"


# ── TEST 6: 传旨前缀剥离
def test_prefix_stripped():
    cmd_create('JJC-TEST-E2E-06', '传旨：帮我写技术博客文章关于智能体架构', 'Strategy', '策枢', '首席战略官')
...
def test_state_update():
    cmd_create('JJC-TEST-E2E-07', '测试状态更新与组织联动功能', 'Strategy', '策枢', '首席战略官')
    cmd_state('JJC-TEST-E2E-07', 'Review', '方案提交衡枢审核')
    t = _get_task('JJC-TEST-E2E-07')
    assert t is not None
    assert t['state'] == 'Review', f"state应为Review: {t['state']}"
    assert t['org'] == '衡枢', f"org应为衡枢: {t['org']}"


# ── TEST 8: done 完成
def test_done():
    cmd_create('JJC-TEST-E2E-08', '测试任务完成状态标记功能', 'Strategy', '策枢', '首席战略官')
...
def test_done_not_overwritable():
    cmd_create('JJC-TEST-E2E-09', '测试已完成任务不可覆盖保护', 'Strategy', '策枢', '首席战略官')
    cmd_done('JJC-TEST-E2E-09', '/tmp/output.md', '任务已完成')
    cmd_create('JJC-TEST-E2E-09', '试图覆盖已完成的任务标题', 'Strategy', '策枢', '首席战略官')
