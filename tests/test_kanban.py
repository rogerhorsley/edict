...
def test_create_and_get(tmp_path):
    ...
    try:
        kb.cmd_create('TEST-001', '测试任务创建和查询功能验证', 'Inbox', '技部', '技术总监')
...
def test_flow_log(tmp_path):
    ...
    tasks_file = tmp_path / 'tasks_source.json'
    tasks_file.write_text(json.dumps([
        {'id': 'T-3', 'title': 'flow test', 'state': 'Strategy', 'flow_log': []}
    ]))

    original = kb.TASKS_FILE
    kb.TASKS_FILE = tasks_file
    try:
        kb.cmd_flow('T-3', '策枢', '衡枢', '方案提交衡枢审核')
        tasks = json.loads(tasks_file.read_text())
        t = tasks[0]
        assert len(t['flow_log']) == 1
        assert t['flow_log'][0]['from'] == '策枢'
        assert t['flow_log'][0]['to'] == '衡枢'
    finally:
        kb.TASKS_FILE = original
...
def test_done(tmp_path):
    ...
    tasks_file = tmp_path / 'tasks_source.json'
    tasks_file.write_text(json.dumps([
        {'id': 'T-4', 'title': 'done test', 'state': 'Doing', 'org': '安部', 'flow_log': []}
    ]))
...
def test_progress(tmp_path):
    ...
    tasks_file = tmp_path / 'tasks_source.json'
    tasks_file.write_text(json.dumps([
        {'id': 'T-5', 'title': 'progress test', 'state': 'Doing', 'org': '技部'}
    ]))
...
def test_progress_log_capped(tmp_path):
    ...
    tasks_file = tmp_path / 'tasks_source.json'
    tasks_file.write_text(json.dumps([
        {'id': 'T-7', 'title': '日志上限测试', 'state': 'Doing', 'org': '品部'}
    ]))
