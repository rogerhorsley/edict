...
Endpoints:
  ...
  GET  /api/pending-insights   → 待审洞察列表
  POST /api/set-model          → {agentId, model}
  POST /api/create-task        → 创建任务（指令下达）
  POST /api/confirm-insight    → 确认洞察并创建任务
  POST /api/reject-insight     → 驳回洞察
  POST /api/issue-command      → 直接下达指令
  POST /api/submit-insight     → 谋部提交主动洞察
...
from detect_openclaw import detect as detect_openclaw, import_items as import_openclaw_items
...
CHANNEL_TYPES = {'feishu', 'telegram', 'wecom', 'discord', 'slack', 'signal', 'tui'}

_WEBHOOK_DOMAIN_WHITELIST = {
    'open.feishu.cn', 'open.larksuite.com', 'api.telegram.org',
    'qyapi.weixin.qq.com', 'discord.com', 'discordapp.com',
    'hooks.slack.com', 'signal.group',
}
...
def _validate_webhook_url(url: str) -> str | None:
    ...
    if not url:
        return None  # empty URL is ok for channels that don't need webhooks (tui)
    parsed = urlparse(url)
    if parsed.scheme not in ('http', 'https'):
        return 'URL must use http or https'
    host = (parsed.hostname or '').lower()
    if host in ('localhost', '127.0.0.1', '0.0.0.0', '::1', ''):
        return 'Cannot use localhost URLs'
    # Block private IPs
    if host.startswith(('10.', '192.168.', '172.')):
        return 'Cannot use private IP addresses'
    if host.startswith('169.254.'):
        return 'Cannot use link-local addresses'
    # Whitelist check (optional — allow any for flexibility)
    return None

CHANNEL_TYPE_META = {
    'feishu':   {'label': '飞书 Feishu', 'icon': '🔷', 'webhook_hint': 'https://open.feishu.cn/open-apis/bot/v2/hook/...'},
    'telegram': {'label': 'Telegram', 'icon': '✈️', 'webhook_hint': 'https://api.telegram.org/bot.../sendMessage'},
    'wecom':    {'label': '企业微信 WeCom', 'icon': '🟢', 'webhook_hint': 'https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=...'},
    'discord':  {'label': 'Discord', 'icon': '💬', 'webhook_hint': 'https://discord.com/api/webhooks/...'},
    'slack':    {'label': 'Slack', 'icon': '💜', 'webhook_hint': 'https://hooks.slack.com/services/...'},
    'signal':   {'label': 'Signal', 'icon': '🔵', 'webhook_hint': ''},
    'tui':      {'label': 'TUI (终端)', 'icon': '🖥️', 'webhook_hint': ''},
}


def load_channels():
    return atomic_json_read(DATA / 'channels.json', {"channels": [], "default_dispatch_channel": None, "default_morning_channel": None})


def save_channels(data):
    atomic_json_write(DATA / 'channels.json', data)


def _test_feishu_webhook(webhook_url: str) -> dict:
    """Send a test message to a Feishu webhook."""
    payload = json.dumps({
        "msg_type": "interactive",
        "card": {
            "header": {"title": {"tag": "plain_text", "content": "三省七部 · 渠道测试"}, "template": "blue"},
            "elements": [{"tag": "div", "text": {"tag": "lark_md", "content": "渠道连接测试成功！此消息来自三省七部看板。"}}]
        }
    }).encode('utf-8')
    try:
        ...
        return {'ok': False, 'error': f"Feishu error: {body.get('msg', str(body))}"}
    except Exception as e:
...
_JWT_SECRET = os.environ.get('AC_JWT_SECRET', '')
if not _JWT_SECRET:
    # Auto-generate a per-instance secret if none is provided (dev mode)
    _JWT_SECRET = secrets.token_hex(32)
    log.warning('AC_JWT_SECRET not set — generated ephemeral secret (tokens will not survive server restart)')
_JWT_EXPIRY_SEC = 86400 * 7  # 7 days
...
def load_users():
    return atomic_json_read(DATA / 'users.json', {"users": [], "sessions": {}})


def save_users(data):
    atomic_json_write(DATA / 'users.json', data)


def _encrypt_api_key(plaintext: str) -> str:
    """Encrypt API key using HMAC-derived key. Not military-grade but avoids plain text."""
    import base64
    key = hmac.new(_JWT_SECRET.encode(), b'api-key-encryption', hashlib.sha256).digest()
    encrypted = bytes(a ^ b for a, b in zip(plaintext.encode(), key * (len(plaintext) // 32 + 1)))
...
def _decrypt_api_key(encrypted: str) -> str:
    ...
    if not encrypted.startswith('enc:'):
        return encrypted  # legacy plain text — return as-is
...
def _get_current_user(handler) -> dict | None:
    ...
    return next((u for u in users_data.get('users', []) if u.get('id') == payload.get('sub')), None)
...
def handle_task_action(task_id, action, reason):
    ...
    if action == 'stop':
        task['state'] = 'Blocked'
        task['block'] = reason or '用户叫停'
        task['now'] = f'⏸️ 已暂停：{reason}'
    elif action == 'cancel':
        task['state'] = 'Cancelled'
        task['block'] = reason or '用户取消'
    ...
    if action == 'resume':
        ...
        _scheduler_add_flow(task, f'用户{action}：{reason or "无"}')
...
def handle_create_task(title, org='谋部', official='战略研究总监', priority='normal', template_id='', params=None, target_dept=''):
    """从看板创建新任务（指令下达）。"""
    ...
    # 清理常见前缀
    title = re.sub(r'^(传旨|下旨|指令)[：:\uff1a]\s*', '', title)
    if len(title) > 100:
        title = title[:100] + '…'
    # 标题质量校验：防止闲聊被误建为任务
    ...
    if title.lower() in _JUNK_TITLES:
        return {'ok': False, 'error': f'「{title}」不是有效指令，请输入具体工作内容'}
    ...
    # 流程起点：用户 -> 谋部研究
    # target_dept 记录模板建议的最终执行部门（仅供行枢派发参考）
    initial_org = '谋部'
    new_task = {
        'id': task_id,
        'title': title,
    ...
    dispatch_for_state(task_id, new_task, 'Research', trigger='user-command')

    return {'ok': True, 'taskId': task_id, 'message': f'指令 {task_id} 已下达，正在派发给谋部'}
...
def handle_review_action(task_id, action, comment=''):
    ...
    if task.get('state') != 'Review':
        return {'ok': False, 'error': f'任务 {task_id} 当前状态为 {task.get("state")}，无法审核'}
    ...
    if action == 'approve':
        ...
        task['now'] = '衡枢通过，移交行枢派发'
        remark = f'✅ 通过：{comment or "衡枢审核通过"}'
        to_dept = '行枢'
    elif action == 'reject':
        round_num = (task.get('review_round') or 0) + 1
        task['review_round'] = round_num
        task['state'] = 'Strategy'
        task['now'] = f'驳回退回策枢修订（第{round_num}轮）'
    ...
    _scheduler_mark_progress(task, f'审核动作 {action} -> {task.get("state")}')
    task['updatedAt'] = now_iso()
    save_tasks(tasks)
...
# ══ 指令下达：Pending Insights ══

INSIGHTS_FILE = DATA / 'pending_insights.json'


def _load_insights():
    return atomic_json_read(INSIGHTS_FILE, [])


def _save_insights(insights):
    atomic_json_write(INSIGHTS_FILE, insights)


def handle_get_pending_insights():
    """获取所有待审洞察（status=pending）。"""
    insights = _load_insights()
    pending = [i for i in insights if i.get('status') == 'pending']
    return {'ok': True, 'insights': pending, 'total': len(pending)}


def handle_confirm_insight(insight_id, edited_title=None):
    """确认洞察并创建任务。"""
    if not insight_id:
        return {'ok': False, 'error': 'insightId required'}

    insights = _load_insights()
    insight = next((i for i in insights if i.get('id') == insight_id), None)
    if not insight:
        return {'ok': False, 'error': f'洞察 {insight_id} 不存在'}
    if insight.get('status') != 'pending':
        return {'ok': False, 'error': f'洞察 {insight_id} 状态为 {insight.get("status")}，无法确认'}

    title = (edited_title or insight.get('title', '')).strip()
    if not title:
        return {'ok': False, 'error': '标题不能为空'}

    # 创建任务
    result = handle_create_task(title, org='谋部', official='战略研究总监')
    if not result.get('ok'):
        return result

    task_id = result['taskId']
    insight['status'] = 'confirmed'
    insight['confirmedAt'] = now_iso()
    insight['taskId'] = task_id
    _save_insights(insights)
    log.info(f'洞察 {insight_id} 已确认，创建任务 {task_id}')
    return {'ok': True, 'taskId': task_id, 'message': f'洞察已确认，任务 {task_id} 已创建'}


def handle_reject_insight(insight_id, reason=''):
    """驳回洞察。"""
    if not insight_id:
        return {'ok': False, 'error': 'insightId required'}
    ...
    if not insight:
        return {'ok': False, 'error': f'洞察 {insight_id} 不存在'}
    if insight.get('status') != 'pending':
        return {'ok': False, 'error': f'洞察 {insight_id} 状态为 {insight.get("status")}，无法驳回'}

    insight['status'] = 'rejected'
    insight['rejectedAt'] = now_iso()
    if reason:
        insight['rejectReason'] = reason
    _save_insights(insights)
    log.info(f'洞察 {insight_id} 已驳回: {reason or "(无理由)"}')
    return {'ok': True, 'message': f'洞察 {insight_id} 已驳回'}


def handle_issue_command(title, description=''):
    """用户直接下达指令（创建 Research 状态任务）。"""
    if not title or not title.strip():
        return {'ok': False, 'error': '指令标题不能为空'}
    result = handle_create_task(title.strip(), org='谋部', official='战略研究总监')
    return result


def handle_submit_insight(title, summary='', source='', suggested_action=''):
    """谋部 Agent 提交主动洞察（内部接口）。"""
    if not title or not title.strip():
        return {'ok': False, 'error': 'title required'}

    today = datetime.datetime.now().strftime('%Y%m%d')
    insights = _load_insights()
    today_ids = [i['id'] for i in insights if i.get('id', '').startswith(f'INS-{today}-')]
    seq = 1
    if today_ids:
        nums = [int(iid.split('-')[-1]) for iid in today_ids if iid.split('-')[-1].isdigit()]
        seq = max(nums) + 1 if nums else 1
    insight_id = f'INS-{today}-{seq:03d}'

    new_insight = {
        'id': insight_id,
        'title': title.strip(),
        'summary': (summary or '').strip(),
        'source': (source or 'research-agent').strip(),
        'suggestedAction': (suggested_action or '').strip(),
        'createdAt': now_iso(),
        'status': 'pending',
        'confirmedAt': None,
        'rejectedAt': None,
    ...
    _save_insights(insights)
    log.info(f'谋部提交洞察: {insight_id} | {title[:40]}')
    return {'ok': True, 'insightId': insight_id, 'message': f'洞察 {insight_id} 已提交，等待用户审批'}


# ══ Agent 在线状态检测 ══

_AGENT_DEPTS = [
    {'id':'research',  'label':'谋部','emoji':'🔭', 'role':'战略研究总监','rank':'VP'},
    {'id':'strategy',  'label':'策枢','emoji':'🧠', 'role':'首席战略官',  'rank':'C-Suite'},
    {'id':'review',    'label':'衡枢','emoji':'⚖️', 'role':'首席审核官',  'rank':'C-Suite'},
    {'id':'execution', 'label':'行枢','emoji':'🎯', 'role':'首席运营官',  'rank':'C-Suite'},
    {'id':'hr',        'label':'人部','emoji':'👥', 'role':'人才总监',    'rank':'Director'},
    {'id':'finance',   'label':'财部','emoji':'💰', 'role':'财务总监',    'rank':'Director'},
    {'id':'brand',     'label':'品部','emoji':'📣', 'role':'品牌总监',    'rank':'Director'},
    {'id':'security',  'label':'安部','emoji':'🛡️', 'role':'安全总监',    'rank':'Director'},
    {'id':'compliance','label':'规部','emoji':'📋', 'role':'合规总监',    'rank':'Director'},
    {'id':'tech',      'label':'技部','emoji':'⚙️', 'role':'技术总监',    'rank':'Director'},
]
...
_STATE_AGENT_MAP = {
    ...
    'Review': 'review',
    'Execution': 'execution',
    'Doing': None,         # 七部，需从 org 推断
    'Pending': 'research', # 待处理，默认谋部
}
_ORG_AGENT_MAP = {
    '谋部': 'research', '人部': 'hr', '财部': 'finance',
    '品部': 'brand', '安部': 'security', '规部': 'compliance', '技部': 'tech',
    '策枢': 'strategy', '衡枢': 'review', '行枢': 'execution',
}
...
def handle_scheduler_retry(task_id, reason=''):
    ...
    sched['lastDispatchTrigger'] = 'system-retry'
...
def handle_scheduler_escalate(task_id, reason=''):
    ...
    target = 'review' if next_level == 1 else 'execution'
    target_label = '衡枢' if next_level == 1 else '行枢'
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    task['now'] = f'↩️ 系统调度自动回滚：{reason or "恢复到上个稳定节点"}'
    task['block'] = '无'
    sched['retryCount'] = 0
    sched['escalationLevel'] = 0
    sched['stallSince'] = None
    ...
    if snap_state not in _TERMINAL_STATES:
        dispatch_for_state(task_id, task, snap_state, trigger='system-rollback')
...
def handle_scheduler_scan(threshold_sec=600):
    ...
    for task in tasks:
        ...
        if retry_count < max_retry:
            ...
            sched['lastRetryAt'] = now_iso()
            sched['lastDispatchTrigger'] = 'system-scan-retry'
    ...
    for task_id, state in pending_retries:
        ...
        if retry_task:
            dispatch_for_state(task_id, retry_task, state, trigger='system-scan-retry')
    ...
    for task_id, state in pending_rollbacks:
        ...
        if rollback_task and state not in _TERMINAL_STATES:
            dispatch_for_state(task_id, rollback_task, state, trigger='system-auto-rollback')
...
# 状态推进顺序（手动推进用）
_STATE_FLOW = {
    'Pending':   ('Research',  '用户', '谋部', '指令下达，谋部开始研究分析'),
    'Research':  ('Strategy',  '谋部', '策枢', '谋部研究完成，提交策枢制定方案'),
    'Strategy':  ('Review',    '策枢', '衡枢', '策枢方案提交衡枢审核'),
    'Review':    ('Execution', '衡枢', '行枢', '衡枢通过，转行枢派发'),
    'Execution': ('Doing',     '行枢', '七部', '行枢开始派发执行'),
    'Doing':     ('Done',      '七部', '完成', '各部完成，任务结束'),
}
_STATE_LABELS = {
    'Pending': '待处理', 'Research': '谋部', 'Strategy': '策枢', 'Review': '衡枢',
    'Execution': '行枢', 'Doing': '执行中', 'Done': '完成',
}
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    _msgs = {
        ...
        'strategy': (
            ...
            f'请立即制定执行方案，走完完整流程（策枢策划→衡枢审核→行枢派发→七部执行）。'
        ),
        'review': (
            f'⚖️ 策枢方案提交审核\n'
            f'任务ID: {task_id}\n'
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif p == '/api/pending-insights':
            self.send_json(handle_get_pending_insights())
        ...
        # ── Channel Config ──
            ...
            self.send_json({'ok': True, **load_channels(), 'types': CHANNEL_TYPE_META})
        ...
        # ── Auth ──
            ...
            has_personal_key = bool(user and user.get('api_key'))
            self.send_json({'ok': True, 'models': models, 'hasPersonalKey': has_personal_key})
    ...
    def do_POST(self):
        ...
        if p == '/api/task-action':
            ...
            reason = body.get('reason', '').strip() or f'用户从看板{action}'
        ...
        if p == '/api/create-task':
            ...
            org = body.get('org', '谋部').strip()
            official = body.get('official', '战略研究总监').strip()
        ...
        if p == '/api/confirm-insight':
            insight_id = body.get('insightId', '').strip()
            edited_title = body.get('editedTitle', '').strip() or None
            if not insight_id:
                self.send_json({'ok': False, 'error': 'insightId required'}, 400)
                return
            result = handle_confirm_insight(insight_id, edited_title)
            self.send_json(result)
            return

        if p == '/api/reject-insight':
            insight_id = body.get('insightId', '').strip()
            reason = body.get('reason', '').strip()
            if not insight_id:
                self.send_json({'ok': False, 'error': 'insightId required'}, 400)
                return
            result = handle_reject_insight(insight_id, reason)
            self.send_json(result)
            return

        if p == '/api/issue-command':
            title = body.get('title', '').strip()
            description = body.get('description', '').strip()
            if not title:
                self.send_json({'ok': False, 'error': 'title required'}, 400)
                return
            result = handle_issue_command(title, description)
            self.send_json(result)
            return

        if p == '/api/submit-insight':
            title = body.get('title', '').strip()
            summary = body.get('summary', '').strip()
            source = body.get('source', '').strip()
            suggested_action = body.get('suggestedAction', '').strip()
            if not title:
                self.send_json({'ok': False, 'error': 'title required'}, 400)
                return
            result = handle_submit_insight(title, summary, source, suggested_action)
            self.send_json(result)
            return
        ...
        # ── OpenClaw Import ──
        elif p == '/api/openclaw-import':
            source_path = body.get('sourcePath', '').strip()
            items = body.get('items', [])
            if not source_path or not items:
                self.send_json({'ok': False, 'error': 'sourcePath and items required'}, 400)
                return
            try:
                result = import_openclaw_items(source_path, items)
                self.send_json(result)
            except Exception as e:
                log.error(f'OpenClaw import failed: {e}', exc_info=True)
                self.send_json({'ok': False, 'error': 'Import failed'}, 500)

        # ── Channel CRUD ──
        elif p == '/api/channels' and self.command == 'POST':
            ch_type = body.get('type', '').strip()
            ch_name = body.get('name', '').strip()
            webhook_url = body.get('webhook_url', '').strip()
            purposes = body.get('purposes', [])
            if ch_type not in CHANNEL_TYPES:
                self.send_json({'ok': False, 'error': f'type must be one of: {", ".join(sorted(CHANNEL_TYPES))}'}, 400)
                return
            if not ch_name:
                self.send_json({'ok': False, 'error': 'name required'}, 400)
                return
            url_err = _validate_webhook_url(webhook_url)
            if url_err:
                self.send_json({'ok': False, 'error': f'Invalid webhook URL: {url_err}'}, 400)
                return
            ch_id = f"{ch_type}-{uuid.uuid4().hex[:8]}"
            new_channel = {
                'id': ch_id,
                'type': ch_type,
                'name': ch_name,
                ...
                'enabled': True,
                'purposes': purposes if isinstance(purposes, list) else [],
                'created_at': now_iso(),
            }
            def _add_channel(data):
                data.setdefault('channels', []).append(new_channel)
                return data
            atomic_json_update(DATA / 'channels.json', _add_channel, {"channels": []})
            self.send_json({'ok': True, 'channel': new_channel})

        elif p == '/api/channels/set-default':
            dispatch = body.get('default_dispatch_channel')
            morning = body.get('default_morning_channel')
            def _set_defaults(data):
                if dispatch is not None:
                    data['default_dispatch_channel'] = dispatch
                if morning is not None:
                    data['default_morning_channel'] = morning
                return data
            atomic_json_update(DATA / 'channels.json', _set_defaults, {"channels": []})
            self.send_json({'ok': True, 'message': '默认渠道已更新'})

        elif p.startswith('/api/channels/') and p.endswith('/test'):
            ch_id = p.replace('/api/channels/', '').replace('/test', '')
            channels_data = load_channels()
            ch = next((c for c in channels_data.get('channels', []) if c.get('id') == ch_id), None)
            if not ch:
                self.send_json({'ok': False, 'error': f'Channel {ch_id} not found'}, 404)
                return
            if ch.get('type') == 'feishu' and ch.get('webhook_url'):
                result = _test_feishu_webhook(ch['webhook_url'])
                self.send_json(result)
            else:
                self.send_json({'ok': False, 'error': f'Test not supported for type: {ch.get("type")}'})

        elif p.startswith('/api/channels/') and not p.endswith('/test'):
            ch_id = p.replace('/api/channels/', '')
            channels_data = load_channels()
            ch_list = channels_data.get('channels', [])
            ch = next((c for c in ch_list if c.get('id') == ch_id), None)
            if not ch:
                self.send_json({'ok': False, 'error': f'Channel {ch_id} not found'}, 404)
                return
            # Update channel fields
            for field in ('name', 'webhook_url', 'enabled', 'purposes'):
                if field in body:
                    ch[field] = body[field]
            ch['updated_at'] = now_iso()
            save_channels(channels_data)
            self.send_json({'ok': True, 'channel': ch})

        # ── Auth endpoints ──
        elif p == '/api/auth/login':
            hc_token = body.get('happycapy_token', '').strip()
            # Also support direct email/name login for local dev
            email = body.get('email', '').strip()
            name = body.get('name', '').strip()
            if not hc_token and not email:
                self.send_json({'ok': False, 'error': 'happycapy_token or email required'}, 400)
                return
            users_data = load_users()
            user = None
            if hc_token:
                # Validate with HappyCapy API
                hc_validated = None
                hc_api_base = os.environ.get('CAPY_BASE_URL', '')
                hc_secret = os.environ.get('CAPY_SECRET', '')
                if hc_api_base and hc_secret:
                    try:
                        req = Request(
                            f'{hc_api_base}/api/auth/validate',
                            data=json.dumps({'token': hc_token}).encode(),
                            headers={'Authorization': f'Bearer {hc_secret}', 'Content-Type': 'application/json'},
                            method='POST',
                        )
                        resp = urlopen(req, timeout=10)
                        hc_validated = json.loads(resp.read())
                    except Exception as e:
                        log.warning(f'HappyCapy API validation failed: {e}')
                # Fall back to token hash if API validation unavailable
                if hc_validated and hc_validated.get('ok'):
                    hc_id = hc_validated.get('user_id', hashlib.sha256(hc_token.encode()).hexdigest()[:16])
                    email = hc_validated.get('email', email)
                    name = hc_validated.get('name', name)
                else:
                    if hc_api_base:
                        log.warning('HappyCapy token validation failed, using hash fallback')
                    hc_id = hashlib.sha256(hc_token.encode()).hexdigest()[:16]
                user = next((u for u in users_data.get('users', []) if u.get('happycapy_id') == hc_id), None)
                if not user:
                    user = {
                        'id': f'usr_{uuid.uuid4().hex[:12]}',
                        'happycapy_id': hc_id,
                        'email': email or f'{hc_id}@happycapy.ai',
                        'name': name or f'User-{hc_id[:6]}',
                        'api_key': None,
                        'model_endpoint': None,
            ...
            self.send_json({'ok': True, 'message': '已登出'})

        elif p == '/api/auth/api-key':
            user = _get_current_user(self)
            if not user:
            ...
            model_endpoint = body.get('model_endpoint', '').strip()
            preferred_model = body.get('preferred_model', '').strip()
            users_data = load_users()
            for u in users_data.get('users', []):
                if u['id'] == user['id']:
                    if api_key:
                        u['api_key'] = _encrypt_api_key(api_key)
                    if model_endpoint:
                        u['model_endpoint'] = model_endpoint
                    if preferred_model:
                        u['preferred_model'] = preferred_model
                    if provider:
                        u['provider'] = provider
                    break
            save_users(users_data)
    ...
    def do_DELETE(self):
        ...
        if p.startswith('/api/channels/'):
            ...
            ch_list = channels_data.get('channels', [])
            before = len(ch_list)
            channels_data['channels'] = [c for c in ch_list if c.get('id') != ch_id]
            if len(channels_data['channels']) == before:
                self.send_json({'ok': False, 'error': f'Channel {ch_id} not found'}, 404)
                return
            # Clear defaults if they pointed to the deleted channel
            if channels_data.get('default_dispatch_channel') == ch_id:
                channels_data['default_dispatch_channel'] = None
            if channels_data.get('default_morning_channel') == ch_id:
                channels_data['default_morning_channel'] = None
            save_channels(channels_data)
            self.send_json({'ok': True, 'message': f'渠道 {ch_id} 已删除'})
        else:
            self.send_error(404)


def main():
    parser = argparse.ArgumentParser(description='三省七部看板服务器')
    ...
    log.info(f'三省七部看板启动 → http://{args.host}:{args.port}')
    print(f'   按 Ctrl+C 停止')
