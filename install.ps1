# ══════════════════════════════════════════════════════════════
# 三省七部 · OpenClaw Multi-Agent System 一键安装脚本 (Windows)
...
function Write-Banner {
    ...
    Write-Host "║  🏛️  三省七部 · OpenClaw Multi-Agent     ║" -ForegroundColor Blue
...
function Build-Frontend {
    ...
    if (-not $node) {
        ...
        Warn "请安装 Node.js 18+ 后运行: cd agents_company_for_happycapy\frontend && npm install && npm run build"
        return
    }
    $pkgJson = Join-Path $REPO_DIR "app\frontend\package.json"
    if (Test-Path $pkgJson) {
        Push-Location (Join-Path $REPO_DIR "app\frontend")
...
Write-Host "║  🎉  三省七部安装完成！                          ║" -ForegroundColor Green
Write-Host "╚══════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "下一步："
Write-Host "  1. 配置 API Key（如尚未配置）:"
Write-Host "     openclaw agents add research  # 按提示输入 Anthropic API Key"
