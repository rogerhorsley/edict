...
监听 topic:
- task.created → 自动派发给 research agent
- task.planning.complete → 策枢策划完成 → 流转衡枢审核
- task.review.result → 衡枢审核 → 通过则 Execution，退回则 Replan
...
log = logging.getLogger("ac.orchestrator")
...
class OrchestratorWorker:
    ...
    async def _on_task_created(self, payload: dict, trace_id: str):
        """任务创建 → 派发给 research agent 研究。"""
        task_id = payload.get("task_id")
        state = payload.get("state", "Pending")
        agent = STATE_AGENT_MAP.get(TaskState(state), "research")
    ...
    async def _on_task_status(self, event_type: str, payload: dict, trace_id: str):
        ...
        # 如果进入 Execution 状态，需要查找七部对应 agent
