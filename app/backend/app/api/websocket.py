...
async def websocket_endpoint(ws: WebSocket):
    ...
    # 订阅所有 系统频道
    await pubsub.psubscribe("ac:pubsub:*")
    ...
    try:
        ...
        await pubsub.punsubscribe("ac:pubsub:*")
...
async def _relay_events(pubsub, ws: WebSocket):
    ...
    async for message in pubsub.listen():
        if message["type"] == "pmessage":
            ...
            # 提取 topic 名
            topic = channel.replace("ac:pubsub:", "") if channel.startswith("ac:pubsub:") else channel
            ...
            if message["type"] == "pmessage":
                ...
                try:
                    ...
                    if payload.get("task_id") == task_id:
                        topic = message["channel"].replace("ac:pubsub:", "")
                        await ws.send_json({
                            "type": "event",
                            "topic": topic,
                            "data": event_data,
