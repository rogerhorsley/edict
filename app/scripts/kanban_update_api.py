...
保持与旧版完全相同的 CLI 接口，内部改为调用 REST API。
如果 API 不可用，降级回写 JSON 文件（过渡期保障）。

用法（与旧版 100% 兼容）:
  python3 kanban_update.py create JJC-20260223-012 "任务标题" Research 谋部 战略研究总监
  python3 kanban_update.py state JJC-20260223-012 Strategy "研究完成，提交策枢策划"
  python3 kanban_update.py flow JJC-20260223-012 "谋部" "策枢" "研究报告提交策枢"
...
# REST API 地址 — 环境变量 > 默认 localhost:8000
AC_API_URL = os.environ.get('AC_API_URL', 'http://localhost:8000')

# 是否启用 API 模式（AC_MODE=api | json | auto）
AC_MODE = os.environ.get('AC_MODE', 'auto').lower()
...
STATE_ORG_MAP = {
    'Research': '谋部', 'Strategy': '策枢', 'Review': '衡枢',
    'Execution': '行枢', 'Doing': '执行中', 'Done': '完成', 'Blocked': '阻塞',
}

# State → Agents Company TaskState value 映射
_STATE_MAP = {
    'Pending': 'Pending', 'Research': 'Research', 'Strategy': 'Strategy',
    'Review': 'Review', 'Execution': 'Execution', 'Doing': 'Doing',
...
def _api_available() -> bool:
    ...
    try:
        ...
        req = urllib.request.Request(f"{AC_API_URL}/health", method='GET')
...
def _check_api():
    ...
    if _api_ok is None:
        ...
        if _api_ok:
            log.debug('REST API 可用，使用 API 模式')
        else:
            log.debug('REST API 不可用，降级到 JSON 模式')
    return _api_ok
...
def cmd_create(task_id, title, state, org, official, remark=None):
    ...
    if _check_api():
        api_state = _STATE_MAP.get(state, state.lower())
        result = _api_post('/api/tasks', {
            'title': title,
            'description': remark or f'任务：{title}',
            'priority': '中',
            'assignee_org': org,
            'creator': official,
            'tags': [task_id],
        ...
        if result:
            log.info(f'✅ 创建 {task_id} → API {result.get("task_id", "?")} | {title[:30]}')
            return
...
def cmd_state(task_id, new_state, now_text=None):
    if _check_api():
        api_state = _STATE_MAP.get(new_state, new_state.lower())
        agent = _infer_agent_id()
        # 需要先通过 legacy_id 查找 task_id
        # 暂用 legacy_id tag 搜索
