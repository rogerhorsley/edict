...
读取旧版 data/tasks_source.json，导入到 Agents Company Postgres 数据库。
...
# 旧版状态 → Agents Company TaskState（含旧版兼容映射）
STATE_MAP = {
    "Pending": TaskState.Pending,
    "Research": TaskState.Research,
    "Strategy": TaskState.Strategy,
    ...
    # 旧版状态兼容映射
    "Taizi": TaskState.Pending,
    "Zhongshu": TaskState.Strategy,
    "Menxia": TaskState.Review,
    "Assigned": TaskState.Execution,
    "Next": TaskState.Doing,
    "Inbox": TaskState.Pending,
    "": TaskState.Pending,
}


def parse_old_task(old: dict) -> dict:
    """将旧版 task JSON 转换为 Agents Company Task 参数。"""
    state_str = old.get("state", "Pending")
    state = STATE_MAP.get(state_str, TaskState.Pending)
