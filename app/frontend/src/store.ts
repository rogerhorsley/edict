...
export const PIPE = [
  { key: 'Inbox',     dept: '用户',  icon: '👤', action: '下达' },
  { key: 'Research',  dept: '谋部',  icon: '🔭', action: '研究' },
  { key: 'Strategy',  dept: '策枢',  icon: '🧠', action: '策划' },
  { key: 'Review',    dept: '衡枢',  icon: '⚖️', action: '审核' },
  { key: 'Execution', dept: '行枢',  icon: '🎯', action: '派发' },
  { key: 'Doing',     dept: '七部',  icon: '⚙️', action: '执行' },
  { key: 'Done',      dept: '完成',  icon: '✅', action: '完成' },
] as const;

export const PIPE_STATE_IDX: Record<string, number> = {
  Inbox: 0, Pending: 0, Research: 1, Strategy: 2, Review: 3,
  Execution: 4, Doing: 5, Done: 6, Blocked: 5, Cancelled: 5,
};

export const DEPT_COLOR: Record<string, string> = {
  '策枢': '#a07aff', '衡枢': '#6a9eff', '行枢': '#6aef9a',
  '谋部': '#e8a040', '人部': '#9b59b6', '财部': '#ff9a6a',
  '品部': '#f5c842', '安部': '#ff5270', '规部': '#cc4444',
  '技部': '#44aaff', '用户': '#ffd700', '完成': '#2ecc8a',
};

export const STATE_LABEL: Record<string, string> = {
  Inbox: '收件', Pending: '待处理', Research: '谋部研究', Strategy: '策枢策划',
  Review: '衡枢审核', Execution: '行枢派发', Doing: '执行中',
...
export function stateLabel(t: Task): string {
  ...
  if (t.state === 'Review' && r > 1) return `衡枢审核（第${r}轮）`;
  if (t.state === 'Strategy' && r > 0) return `策枢修订（第${r}轮）`;
  return STATE_LABEL[t.state] || t.state;
}

export function isMainTask(t: Task): boolean {
...
export type TabKey =
  | 'tasks' | 'command' | 'monitor' | 'officials' | 'models'
  | 'skills' | 'sessions' | 'memorials' | 'templates' | 'morning' | 'court' | 'setup';

export const TAB_DEFS: { key: TabKey; label: string; icon: string }[] = [
  { key: 'tasks',    label: '任务看板', icon: '📋' },
  { key: 'command',   label: '指令下达', icon: '📡' },
  { key: 'court',     label: '圆桌议事', icon: '🏛️' },
  { key: 'monitor',   label: '枢部调度', icon: '🔌' },
  { key: 'officials', label: '部门总览', icon: '👔' },
  { key: 'models',    label: '模型配置', icon: '🤖' },
  { key: 'skills',    label: '技能配置', icon: '🎯' },
  { key: 'sessions',  label: '小任务',   icon: '💬' },
  { key: 'memorials', label: '归档',     icon: '📜' },
  { key: 'templates', label: '模板库',   icon: '📋' },
  { key: 'morning',   label: '行业资讯', icon: '🌅' },
  { key: 'setup',     label: '系统设置', icon: '⚙️' },
];
...
export const TEMPLATES: Template[] = [
  ...
  },
  {
    ...
    depts: ['谋部', '财部', '品部'], est: '~60分钟', cost: '¥5',
    params: [
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  login: async (token: string) => {
    ...
    try {
      ...
      if (r.ok && r.user) {
        set({ user: r.user, isAuthenticated: true });
      }
    } catch {
      // silently fail
    }
  ...
  loadInsights: async () => {
    try {
      const data = await api.pendingInsights();
      set({ pendingInsights: data.insights || [] });
    } catch {
      // silently fail
    }
  },
