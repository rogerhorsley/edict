...
import { useStore, isMainTask, STATE_LABEL } from '../store';
...
export default function MemorialPanel() {
  ...
  let mems = tasks.filter((t) => isMainTask(t) && ['Done', 'Cancelled'].includes(t.state));
  ...
  return (
    <div>
      ...
      <div className="mem-list">
        {!mems.length ? (
          <div className="mem-empty">暂无交付报告 — 任务完成后自动生成</div>
        ) : (
          mems.map((t) => {
            const fl = t.flow_log || [];
...
function MemorialDetailModal({
  ...
  for (const f of fl) {
    ...
    else if (f.to === '策枢' || f.from === '策枢') planLog.push(f);
    else if (f.to === '衡枢' || f.from === '衡枢') reviewLog.push(f);
    ...
    <div className="modal-bg open" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {renderPhase('策枢规划', '🧠', planLog)}
          {renderPhase('衡枢审核', '⚖️', reviewLog)}
          {renderPhase('七部执行', '⚙️', execLog)}
          {renderPhase('汇总完成', '📨', resultLog)}
