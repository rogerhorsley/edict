...
const OFFICIAL_COLORS: Record<string, string> = {
  research: '#e8a040', strategy: '#a07aff', review: '#6a9eff', execution: '#2ecc8a',
  hr: '#9b59b6', finance: '#ff9a6a', brand: '#f5c842', security: '#ff5270',
  compliance: '#cc4444', tech: '#44aaff',
};
...
const COURT_POSITIONS: Record<string, { x: number; y: number }> = {
  // 左列 - 三枢
  strategy: { x: 15, y: 25 }, review: { x: 15, y: 45 }, execution: { x: 15, y: 65 },
  // 右列 - 七部
  hr: { x: 85, y: 15 }, finance: { x: 85, y: 30 }, brand: { x: 85, y: 45 },
  security: { x: 85, y: 60 }, compliance: { x: 85, y: 75 }, tech: { x: 85, y: 90 },
  // 中间
  research: { x: 50, y: 20 },
};
...
export default function CourtDiscussion() {
        ...
        setSession((prev) =>
          prev
            ? {
              ...
              messages: [
                ...prev.messages,
                { type: 'system', content: `📋 圆桌议事结束 — ${res.summary}`, timestamp: Date.now() / 1000 },
              ],
            }
            : prev,
        );
  ...
  // ── 预设议题（从当前任务中提取）──
  const activeTasks = (liveStatus?.tasks || []).filter(
      ...
      <div className="space-y-6">
        ...
        <div className="text-center py-4">
          ...
          <p className="text-xs text-[var(--muted)] mt-1">
            选择参会成员，围绕议题展开讨论 · 用户可随时发言或干预改变走向
          </p>
        </div>
        ...
        >
          {loading ? '召集中...' : `🏛 开始议事（${selectedIds.size}位参会）`}
        </button>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 bg-[var(--panel)] rounded-xl px-4 py-2 border border-[var(--line)]">
        <div className="flex items-center gap-2">
          <span className="text-sm font-bold">🏛 圆桌议事</span>
        ...
        <div className="bg-[var(--panel)] rounded-xl p-3 border border-[var(--line)] relative overflow-hidden min-h-[320px]">
          ...
          <div className="text-center mb-2">
            <div className="inline-block px-3 py-1 rounded-lg bg-gradient-to-b from-amber-800/40 to-amber-950/40 border border-amber-700/30">
              <span className="text-lg">👑</span>
              <div className="text-[10px] text-amber-400/80">主 席</div>
            </div>
          </div>
          ...
          <div className="relative" style={{ minHeight: 250 }}>
            {/* 左列标签 */}
            <div className="absolute left-0 top-0 text-[9px] text-[var(--muted)] opacity-50">三枢</div>
            <div className="absolute right-0 top-0 text-[9px] text-[var(--muted)] opacity-50">七部</div>
    ...
    return (
      <div className="text-center py-2">
        <div className="inline-block bg-gradient-to-r from-amber-900/30 via-purple-900/30 to-amber-900/30 rounded-lg px-4 py-2 border border-amber-600/30">
          <div className="text-xs text-amber-400 font-bold">⚡ 主持人干预</div>
