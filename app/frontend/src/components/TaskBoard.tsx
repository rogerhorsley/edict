import { useStore, isMainTask, isArchived, getPipeStatus, stateLabel, deptColor, PIPE } from '../store';
import { api, type Task } from '../api';

// 排序权重
const STATE_ORDER: Record<string, number> = {
  Doing: 0, Execution: 1, Review: 2, Strategy: 3, Research: 4,
  Pending: 5, Inbox: 6, Blocked: 7, Done: 8, Cancelled: 9,
};
...
function TaskCard({ task }: { task: Task }) {
  ...
  return (
    <div
      className={`task-card${archived ? ' archived' : ''}`}
...
export default function TaskBoard() {
  ...
  const allTasks = tasks.filter(isMainTask);
  const activeTasks = allTasks.filter((t) => !isArchived(t));
  const archivedTasks = allTasks.filter((t) => isArchived(t));

  let filtered: Task[];
  if (taskFilter === 'active') filtered = activeTasks;
  else if (taskFilter === 'archived') filtered = archivedTasks;
  else filtered = allTasks;

  filtered.sort((a, b) => (STATE_ORDER[a.state] ?? 9) - (STATE_ORDER[b.state] ?? 9));
  ...
  const handleScan = async () => {
    try {
      ...
      if (r.ok) toast(`🧭 系统巡检完成：${r.count || 0} 个动作`);
  ...
  return (
    <div>
      ...
      <div className="archive-bar">
        ...
        <button className="ab-scan" onClick={handleScan}>🧭 系统巡检</button>
      </div>
